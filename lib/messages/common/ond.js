...
module.exports = function OriginDestinations(data, root, arrayRoot) {
    ...
    result[arrayRoot] = data.flights.map(function (flight) {
        return {
            ...
            MarketingCarrierAirline: ((['SeatAvailability', 'FlightPrice'].indexOf(root.message) > -1) && (flight.airline || root.sender) ? null : {
                AirlineID: (flight.airline || root.sender).id,
                Name: (flight.airline || root.sender).name
            }),
            MarketingCarrier: ((['SeatAvailability', 'FlightPrice'].indexOf(root.message) > -1) && (flight.airline || root.sender) ? {
                AirlineID: (flight.airline || root.sender).id,
                Name: (flight.airline || root.sender).name,
